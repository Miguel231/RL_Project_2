    ...
    def act(self, obss) -> List[int]:
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for agent in range(self.num_agents):
            ...
            q_values = [q_table[str((obs,a))] for a in range(self.n_acts[agent])]

            #epsilon
            if random.random() < self.epsilon:
                #rand action
                action = random.randint(0,self.n_acts[agent] - 1)
            else:
                #action with max q_value
    ...
    def learn(
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for agent in range(self.num_agents):
            ...
            q_table[str((obs, action))] = current_q + self.learning_rate*(target - current_q)
            updated_qv.append(q_table[str((obs, action))])
        return updated_qv
